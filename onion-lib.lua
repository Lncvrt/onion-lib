...
Onion.Name = 'Onion'
if syn then
    syn.protect_gui(Onion)
    Onion.Parent = game.CoreGui
else
    Onion.Parent = gethui() or game.CoreGui
end

if gethui then
    for _, Interface in ipairs(gethui():GetChildren()) do
        if Interface.Name == Onion.Name and Interface ~= Onion then
            Interface:Destroy()
        end
    end
else
...
function OnionLib:IsRunning()
    if gethui then
        return Onion.Parent == gethui()
    else
...
local function AddConnection(Signal, Function)
    if (not OnionLib:IsRunning()) then
        return
    end
    local SignalConnect = Signal:Connect(Function)
    table.insert(OnionLib.Connections, SignalConnect)
    return SignalConnect
end

task.spawn(function()
    while (OnionLib:IsRunning()) do
        wait()
    end

    for _, Connection in next, OnionLib.Connections do
...
local function CreateElement(ElementName, ElementFunction)
    OnionLib.Elements[ElementName] = function(...)
        return ElementFunction(...)
    end
end

local function MakeElement(ElementName, ...)
    local NewElement = OnionLib.Elements[ElementName](...)
    return NewElement
end
...
function OnionLib:MakeNotification(NotificationConfig)
    spawn(function()
...
function OnionLib:MakeWindow(WindowConfig)
    ...
    WindowConfig.Name = WindowConfig.Name or 'Onion Library'
    ...
    WindowConfig.IntroText = WindowConfig.IntroText or 'Onion Library'
    WindowConfig.CloseCallback = WindowConfig.CloseCallback or function() end
    WindowConfig.ShowIcon = WindowConfig.ShowIcon or false
    WindowConfig.Icon = WindowConfig.Icon or 'rbxassetid://8834748103'
    WindowConfig.IntroIcon = WindowConfig.IntroIcon or 'rbxassetid://8834748103'
    OnionLib.Folder = WindowConfig.ConfigFolder
    OnionLib.SaveCfg = WindowConfig.SaveConfig
